      ...
      <div className='space-y-6'>
        ...
        {(!clientId || !logoutUri || !cognitoDomain) && (
          <section className='card'>
            ...
            <p className='text-slate-700'>
              Set <code>GATSBY_COGNITO_CLIENT_ID</code>, <code>GATSBY_COGNITO_DOMAIN</code>, and{' '}
              <code>GATSBY_COGNITO_REDIRECT_URI</code> (or <code>GATSBY_COGNITO_LOGOUT_URI</code>) to enable sign-in.
            </p>
          </section>
        )}

        <section className='card'>
          {mounted ? <AuthContent /> : <div>Loading...</div>}
        </section>
      </div>
    </Layout>
  );
...
// Separated so useAuth() is never called during SSR — AuthProvider is client-only
function AuthContent() {
  ...
  if (auth.isAuthenticated) {
    return (
      <div className='space-y-3'>
        ...
        <button
          className='inline-flex items-center rounded-full bg-emerald-600 text-white px-4 py-2 text-sm font-semibold'
  ...
  return (
    <div className='flex flex-wrap gap-3'>
      ...
      <button
        className='inline-flex items-center rounded-full border border-emerald-600 text-emerald-700 px-4 py-2 text-sm font-semibold'
